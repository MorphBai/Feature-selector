...
"""
https://ieeexplore.ieee.org/document/9313548
"""
    ...
    def __init__(self, in_channels, ratio=16, dim=2):
        ...
        self.excitation = nn.Sequential(
            nn.Linear(in_features=in_channels,
                      out_features=in_channels // ratio),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=in_channels //
                      ratio, out_features=in_channels),
            nn.Sigmoid()
        )
...
class SE_ECGNet(nn.Module):
    ...
    def __init__(self, struct=[(1, 3), (1, 5), (1, 7)], num_classes=5):
        super().__init__()
        self.struct = struct
        ...
        self.fc = nn.Linear(in_features=384 * len(struct),
    ...
    def forward(self, features, info=None, labels=None):
        out = features.unsqueeze(1)
        out = self.conv(out)  
        out = self.bn(out)
        out = self.relu(out)
        out = self.block1(out)  
        ...
        for i in range(len(self.struct)):
            sep = self.block2_list[i](out)  # x => [b, 32, 12, 60]
            # x => [b, 384, 60]
            sep = sep.reshape(sep.shape[0], -1, sep.shape[3])
            sep = self.block3_list[i](sep)  # x => [b, 384, 4]
            sep = self.avgpool(sep)  # x => [b, 384, 1]
        ...
        logits = torch.sigmoid(self.fc(out))
